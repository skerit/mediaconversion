 ...
 * @param    {String}   name
 */
var Multiplier = Fn.inherits('Informer', 'Develry.MediaConversion', function StreamMultiplier(stream, name) {
	...
	stream.on('data', function onData(chunk) {
		...
		for (i = 0; i < that.target_streams.length; i++) {
			if (that.target_streams[i] && !that.target_streams[i]._writableState.ended) {
				that.target_streams[i].push(chunk);
			}
		}
	...
	// Make sure the streams end on error
 ...
 * @version  0.1.0
 */
Multiplier.setMethod(function destroy(destroy_source) {

	var that = this,
	    i;

	// @todo: add more methods of stopping the stream
	if ((destroy_source == null || destroy_source == true) && this.original_stream.destroy) {
	...
	for (i = 0; i < this.target_streams.length; i++) {
		if (this.target_streams[i]) {
			this.target_streams[i].push(null);
		}
	}
});
...
Multiplier.setMethod(['createStream', 'fork'], function createStream(name) {
	...
	if (this.original_stream && this.original_stream.cloneWithData) {
		result = this.original_stream.cloneWithData({allowHalfOpen: false});
	} else {
		// Create a new passthrough stream
		result = new MC.MediaStream({allowHalfOpen: false});
	}
